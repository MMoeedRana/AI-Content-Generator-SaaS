...
import {
  TrendingUp, Zap, FileText, Lock, Sparkles, Clock, BarChart3
} from "lucide-react";
    ...
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 bg-gray-50 dark:bg-slate-950 min-h-screen">
        ...
        <div className="w-full md:w-auto">
          <h1 className="text-2xl md:text-3xl font-bold flex items-center gap-3 text-slate-900 dark:text-white">
            ...
            <span className="truncate">
              {user?.firstName ? `${user.firstName}'s Analytics` : "Your Analytics"}
            </span>
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-4 md:p-6 rounded-2xl border border-gray-200 dark:border-slate-800 relative shadow-sm transition-all hover:shadow-md overflow-hidden">
          ...
          <div className={`h-[250px] md:h-[300px] w-full ${!isPremium ? "blur-md grayscale opacity-50" : ""}`}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={stats?.weeklyData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                <defs>
                ...
                <Tooltip
                  contentStyle={{
                    ...
                    backgroundColor: "#1e293b", color: "#fff", fontSize: "12px",
                  }}
                  itemStyle={{ color: "#fff" }}
                />
                <Area type="monotone" dataKey="words" stroke="#6366f1" strokeWidth={3} fill="url(#colorWords)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 md:p-6 rounded-2xl border border-gray-200 dark:border-slate-800 shadow-sm relative transition-all hover:shadow-md overflow-hidden">
          <h3 className="font-bold text-base md:text-lg mb-6 dark:text-white">Top Templates</h3>
          {!isPremium && <LockOverlay message="Unlock Category Stats" />}
          <div className={`h-[250px] md:h-[300px] ${!isPremium ? "blur-md grayscale opacity-50" : ""}`}>
            <ResponsiveContainer width="100%" height={180}>
              <PieChart>
                <Pie
                  data={stats?.templateStats} innerRadius={50} outerRadius={70}
            ...
            <div className="mt-4 space-y-2 max-h-[100px] overflow-y-auto pr-2 custom-scrollbar">
              {stats?.templateStats?.map((item: any, i: number) => (
                <div key={i} className="flex justify-between text-[11px] md:text-xs">
                  ...
                  <span className="font-bold dark:text-white shrink-0">{item.value} Gen</span>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-slate-950 px-4 text-center">
      <div className="animate-pulse flex flex-col items-center">
        ...
        <p className="text-gray-500 font-medium text-xl">Gathering your insights...</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-5 md:p-6 rounded-2xl border border-gray-200 dark:border-slate-800 hover:border-primary/50 transition-all group shadow-sm">
      <div className="flex justify-between items-center">
        <div className="p-2 bg-gray-50 dark:bg-slate-800 rounded-lg group-hover:scale-110 transition-transform">{icon}</div>
        <span className="text-[9px] md:text-[10px] font-bold text-primary bg-primary/10 px-2 py-0.5 md:py-1 rounded-full uppercase tracking-wider">{trend}</span>
      </div>
      <div className="mt-4">
        <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 font-medium">{title}</p>
        <h2 className="text-2xl md:text-3xl font-bold mt-1 tracking-tight dark:text-white truncate">{value}</h2>
      </div>
    </div>
  );
}
...
function LockOverlay({ message }: { message: string }) {
  return (
    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/40 dark:bg-slate-900/40 backdrop-blur-[4px] rounded-2xl transition-all p-4">
      <div className="bg-white dark:bg-slate-800 p-5 md:p-6 rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-700 text-center animate-in zoom-in duration-300">
        <div className="bg-primary/10 p-2 rounded-full w-fit mx-auto mb-3"><Lock className="h-5 w-5 text-primary" /></div>
        <h4 className="text-sm font-bold text-gray-900 dark:text-white">{message}</h4>
        <Button size="sm" className="mt-4 w-full bg-primary text-white text-xs">Upgrade Now</Button>
      </div>
    </div>
  );
}